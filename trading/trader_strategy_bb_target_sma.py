...
class BB_to_SMA_Strategy(Strategy):
    ...
    def determine_action(self, bid, ask, units) -> Trade_Action:
        ...
        if units > 0:  # if already have long positions
            ...
            if price > target and self.slope10 < 0:  # if price is above target SMA, SELL
                signal = -1
                # price = bid
                logger.info(
                    f"Close {units} long position - signal SELL at price: {price}, sma: {self.target}, spread: {spread}, slope: {self.slope10}"
                )
        elif units < 0:  # if alredy have short positions
            # target = self.target - spread
            # if ask < target:  # price is below target SMA, BUY
            if price < target and self.slope10 > 0:  # price is below target SMA, BUY
                signal = 1
                # price = ask
                logger.info(
                    f"Close {units} short position - Signal BUY at price: {price}, sma: {self.target}, spread: {spread}, slope: {self.slope10}"
                )
            ...
            if spread >= (self.bb_upper - target):
                ...
                if price < self.bb_lower and self.slope10 > 0: # if price is below lower BB, BUY

                    signal = 1
                    # price = ask
                    logger.info(
                        f"Signal BUY at price: {price}, bb_lower: {self.bb_lower}, spread: {spread}, rsi: {self.rsi}, slope: {self.slope10}"
                    )
                ...
                elif price > self.bb_upper and self.slope10 < 0:  # if price is above upper BB, SELL

                    signal = -1
                    # price = bid
                    logger.info(
                        f"Signal SELL at price: {price}, bb_upper: {self.bb_upper}, spread: {spread}, rsi: {self.rsi}, slope: {self.slope10}"
                    )
