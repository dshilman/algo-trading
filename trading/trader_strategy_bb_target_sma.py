    ...
    def determine_action(self, bid, ask, units) -> Trade_Action:
        ...
        if units > 0:  # if already have long positions
            ...
            if spread >= (self.bb_upper - target):
                ...
                if price < self.bb_lower and self.slope5 > 0: # if price is below lower BB, BUY

                    signal = 1
                    # price = ask
                    logger.info(
                        f"Signal BUY at price: {price}, bb_lower: {self.bb_lower}, spread: {spread}, rsi: {self.rsi}, slope: {self.slope5}"
                    )
                ...
                elif price > self.bb_upper and self.slope5 < 0:  # if price is above upper BB, SELL

                    signal = -1
                    # price = bid
                    logger.info(
                        f"Signal SELL at price: {price}, bb_upper: {self.bb_upper}, spread: {spread}, rsi: {self.rsi}, slope: {self.slope5}"
                    )
