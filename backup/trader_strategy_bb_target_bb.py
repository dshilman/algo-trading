...
logHandler = handlers.RotatingFileHandler('strategy_bb_bb.log', maxBytes=5*1024*1024, backupCount=10)
...
class ConTrader(tpqoa.tpqoa):
    ...
    def define_strategy(self, resampled_tick_data = None): # "strategy-specific"
        ...
        df["distance"] = df[self.instrument] - df.Lower
    ...
    def determine_action(self, bid, ask):
        ...
        if ask < self.bb_lower:
           signal = 1
           price = ask
           logger.info (f"Signal BUY at price: {price}, bb_lower: {self.bb_lower}, spread: {round(bid - ask, 4)}")
    
        elif bid > self.bb_upper:
           signal = -1
           price = bid
           logger.info (f"Signal SELL at price: {price}, bb_upper: {self.bb_upper}, spread: {round(bid - ask, 4)}")
