...
class BollTrader(tpqoa.tpqoa):
    ...
    def on_success(self, time, bid, ask):
        ...
        self.tick_data = pd.concat(
            [self.tick_data, df])  # new with pd.concat()
    ...
    def define_strategy(self):  # "strategy-specific"
        ...
        df["Lower"] = df["SMA"] - \
            df[self.instrument].rolling(self.SMA).std() * self.dev
        df["Upper"] = df["SMA"] + \
            df[self.instrument].rolling(self.SMA).std() * self.dev
        df["distance"] = df[self.instrument] - df.SMA
        df["position"] = np.where(df[self.instrument] < df.Lower, 1, np.nan)
        df["position"] = np.where(
            df[self.instrument] > df.Upper, -1, df["position"])
        df["position"] = np.where(
...
if __name__ == "__main__":
    ...
    trader = BollTrader(r"C:\Users\hagma\AlgoTrading\Part4_Materials\Oanda\oanda.cfg",
                        "EUR_USD", "1min", SMA=20, dev=1, units=100000)
