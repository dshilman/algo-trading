...
class TradingStrategy(strategy_e.TradingStrategy):
    ...
    def check_if_need_open_trade(self, trading_time):
        ...
        if self.short_trading and self.volume == self.volume_max > self.trading_volume and self.volume_pct_change > .5 \
            and self.rsi_short_pct_change < -self.rsi_change and self.rsi_short == self.rsi_short_min < 30 and self.price - self.price_open < 0 \
        ...
        elif self.long_trading and self.volume == self.volume_max > self.trading_volume and self.volume_pct_change > .5 \
            and self.rsi_short_pct_change > self.rsi_change and self.rsi_short == self.rsi_short_max > 70 and self.price - self.price_open > 0 \
